...
format_reset_time() {
    ...
    case "$style" in
        time)
            date -j -r "$epoch" +"%l:%M%p" 2>/dev/null | sed 's/^ //; s/\.//g' || \
            date -d "@$epoch" +"%l:%M%P" 2>/dev/null | sed 's/^ //; s/\.//g'
            ;;
        datetime)
            date -j -r "$epoch" +"%b %-d, %l:%M%p" 2>/dev/null | sed 's/  / /g; s/^ //; s/\.//g' || \
            date -d "@$epoch" +"%b %-d, %l:%M%P" 2>/dev/null | sed 's/  / /g; s/^ //; s/\.//g'
            ;;
        *)
            date -j -r "$epoch" +"%b %-d" 2>/dev/null || \
...
if [ -n "$cost_usd" ] && [ "$cost_usd" != "null" ]; then
    cost_formatted=$(printf '%.2f' "$cost_usd" 2>/dev/null)
    if [ -n "$cost_formatted" ]; then
        # Color cost by dollar thresholds
        cost_cents=$(awk "BEGIN {printf \"%.0f\", $cost_usd * 100}")
        if [ "$cost_cents" -ge 2000 ]; then
            cost_color="$red"
        elif [ "$cost_cents" -ge 800 ]; then
            cost_color="$orange"
        elif [ "$cost_cents" -ge 400 ]; then
            cost_color="$yellow"
        else
            cost_color="$green"
        fi
        cost_display="${cost_color}\$${cost_formatted}${reset}"
    fi
fi

# ── LINE 1: Model │ Context % │ Directory (branch) │ Cost │ Session │ Thinking ──
