...
The swap between variants is a contextual icon animation; use the exact cross-fade values in [interaction guidance](polish-interactions.md).
...
Analyze composite icons part by part: a badge or slash overlay may keep its position even when the base glyph flips. Accessible names for icon-only buttons are covered by the accessibility references.
