...
2. Binary-search the cause. Form the candidate hypotheses, then rule them out until one survives. Seed them with `explain-codebase` How mode over the affected subsystem and Rationale mode for regression history. Each pass, take the split that cuts the most remaining problem space, get runtime evidence, eliminate. When program state is unclear, add instrumentation or logging and read it as the code runs. Don't guess. Drive a long or stubborn hunt with Cursor's `/loop` command. Confirm the surviving *mechanism* with runtime evidence before the step-3 architect/interrogate fan-out.
...
5. Stage the commits so the failing repro lands before the fix in git history; the diff tells the story. See the **tdd** skill for the failing-test-first cadence when the bug has a cheap local test path; skip it when the test would be expensive, integration-heavy, or unclear.
   This is the canonical [Sequence Work into Verifiable Units](../references/principles/sequence-verifiable-units.md) principle reference, the failing test first and the fix on top.
6. Run **Opening a PR**.

Investigation may fan out `explain-codebase` How and Rationale modes in parallel.
